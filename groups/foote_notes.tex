...
\begin{quote}
\emph{Proof:}  
Put ${\cal X} = \{ X^G \}$ and $N=N_R({\cal Y})$.  If $N=R$, every element of ${\cal A} - {\cal Y}$ normalized
...
\begin{quote}
\emph{Proof:}  
Again, put ${\cal X} = \{ X^G \}$.  Let ${\cal Z} \subseteq {\cal X}$ be maximal wrt ${\cal Z} = P \in p(G)$.
If ${\cal Z} = {\cal X}$, we're done. ${\cal Z} \nleqslant {\cal X}$. $\langle {\cal Z}, U \rangle$ is not
a $p$-groups for $U \in {\cal X} - {\cal Z}$.  Over all $p$-groups of $G$ that contain an element of 
...
$\langle {\cal Y} \rangle$.  Put ${\cal U} = \{ X \in {\cal X}: X \subseteq Q \}$.  By the lemma,
...
\end{quote}
{\bf Corollary:} If $z \in Inv(G)$ and $z \notin O_2(G)$, $z$ inverts some element of odd irder in $G$.
