...
\end{quote}
{\bf Theorem 4:} If $P \in S_p(G)$ and $n_p=|G:N_G(P)| \ne 1 \jmod {p^k}$, then $\exists R \in S_p(G) : |P/(P \cap R)| \leq p^{k-1}$.
\begin{quote}
\emph{Proof:} 
Let $O_1, ..., O_k$ be the orbits of $P$ conjugates in $G$ under the $P$ acting by conjugation. $O_1= \{P\}$.
If  $n_p  \ne 1 \jmod {p^k}$,  $\exists R, |P/N_P(R)|  \ne p^k$, so $|P/N_P(R)| \nmid  p^k$.  But $N_P(R) = P \cap R$ so
$\exists R: |P/P \cap R| \leq p^{k-1}$.
\end{quote}
