...
#define ROL(a, offset)                             \
  ((((uint64_t)a) << ((offset) % num_bits_in_uint64_t)) ^ \
   (((uint64_t)a) >> (num_bits_in_uint64_t - ((offset) % num_bits_in_uint64_t))))
...
Sha3::Sha3(int num_bits) { num_out_bytes_ = num_bits / num_bits_in_byte; }

Sha3::~Sha3() {}

void Sha3::TransformBlock(const uint64_t* in, int laneCount) {
  int round;
...
void Sha3::AddToHash(int size, const byte* in) {
  if (num_bytes_waiting_ > 0) {
    ...
    num_bits_processed_ += BLOCKBYTESIZE * num_bits_in_byte;
    size -= needed;
    in += needed;
    num_bytes_waiting_ = 0;
  }
...
bool Sha3::GetDigest(int size, byte* out) {
