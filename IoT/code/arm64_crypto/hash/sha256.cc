...
void Sha256::TransformBlock(const uint32_t* block) {
  uint32_t data[16];
  uint32_t W[16];
  uint32_t T[8];
...
void Sha256::AddToHash(int size, const byte* in) {
...
bool Sha256::GetDigest(int size, byte* out) {
...
void Sha256::Final() {
  uint64_t num_bits = num_bits_processed_ + num_bytes_waiting_ * num_bits_in_byte;
