...
Cmac::Cmac(int num_bits) {
  num_out_bytes_ = (num_bits + num_bits_in_byte - 1) / num_bits_in_byte;
...
void Cmac::AddToHash(int size, const byte* in) {
  ...
  if (num_bytes_waiting_ > 0) {
    ...
    num_bits_processed_ += Aes::BLOCKBYTESIZE * num_bits_in_byte;
    size -= needed;
    in += needed;
    num_bytes_waiting_ = 0;
  }
...
void Cmac::Final(int size, const byte* in) {
  ...
  num_bits_processed_ += size * num_bits_in_byte;
  finalized_ = true;
}
