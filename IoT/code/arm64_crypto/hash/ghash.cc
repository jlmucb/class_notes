...
const int uint64_bit_size = sizeof(uint64_t) * num_bits_in_byte;
...
bool BitOn(uint64_t* in, int pos) {
  ...
  int bit_position = pos - word_position * num_bits_in_uint64_t;
...
void Shift(int size_in, uint64_t* in, int shift, int size_out, uint64_t* out) {
  ...
  for (int i = (size_in - 1); i >= 0; i--) {
    ...
    top_shift = (sizeof(uint64_t) * num_bits_in_byte) - bit_shift;
    if (top_shift != (sizeof(uint64_t) * num_bits_in_byte))
      top = in[i] >> top_shift;
    else
      top = 0ULL;
...
void Ghash::AddAHash(int size, byte* data) {
  size_A_ += size * num_bits_in_byte;
  AddToHash(size, data);
}

void Ghash::AddCHash(int size, byte* data) {
  size_C_ += size * num_bits_in_byte;
  AddToHash(size, data);
}
