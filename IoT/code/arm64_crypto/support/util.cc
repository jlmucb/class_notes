...
void CloseCrypto() {
  if (random_device_desciptor > 0) close(random_device_desciptor);
  random_device_desciptor = -1;
  return;
}

bool GetCryptoRand(int num_bits, byte* buf) {
  int num_bytes = ((num_bits + num_bits_in_byte - 1) / num_bits_in_byte);
...
bool TimePoint::TimePointLaterBy(TimePoint& from, int years_later,
                                 int months_later, int days_later,
                                 int hours_later, int minutes_later,
...
bool TimePoint::TimePointLaterBySeconds(TimePoint& from,
  ...
  return TimePointLaterBy(from, years_later, months_later, days_later,
                          hours_later, minutes_later, seconds_later);
}

void TimePoint::PrintTime() {
  printf("%04d:%02d:%02d.%02d:%02d:%lfZ", year_, month_, day_in_month_, hour_,
         minutes_, seconds_);
}
...
string* EncodeTime(TimePoint the_time) {
  char str_time[128];
  sprintf(str_time, "%04d:%02d:%02d.%02d:%02d:%lfZ", the_time.year_,
          the_time.month_, the_time.day_in_month_, the_time.hour_,
          the_time.minutes_, the_time.seconds_);
...
bool DecodeTime(string encoded_time, TimePoint* the_time) {
  ...
  sscanf(str_time, "%04d:%02d:%02d.%02d:%02d:%lfZ", &the_time->year_,
         &the_time->month_, &the_time->day_in_month_, &the_time->hour_,
         &the_time->minutes_, &the_time->seconds_);
  return true;
}
