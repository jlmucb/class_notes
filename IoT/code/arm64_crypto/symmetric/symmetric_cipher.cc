...
SymmetricCipher::~SymmetricCipher() {
  ...
  num_key_bits_ = 0;
  if (key_ != nullptr) {
    memset(key_, 0, num_key_bits_ / num_bits_in_byte);
    delete key_;
    key_ = nullptr;
  }
}
...
void SymmetricCipher::PrintSymmetricKey() {
  ...
  if (key_ == nullptr) {
    ...
    PrintBytes(num_key_bits_ / num_bits_in_byte, key_);
    printf("\n");
  }
}
...
bool SymmetricCipher::SerializeSymmetricCipherToMessage(
  ...
  string* s = ByteToBase64LeftToRight(num_key_bits_ / num_bits_in_byte, key_);
  if (s == nullptr) {
...
bool SymmetricCipher::DeserializeSymmetricCipherFromMessage(
  ...
  key_ = new byte[num_key_bits_ / num_bits_in_byte];
  if (Base64ToByteLeftToRight((char*)message.value().c_str(),
                              num_key_bits_ / num_bits_in_byte, key_) < 0) {
    LOG(ERROR)
