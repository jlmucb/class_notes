...
Twofish::~Twofish() { memset((byte*)&round_data, 0, sizeof(round_data)); }
...
uint32_t Twofish::h(int k, byte L[], int kCycles) {
  switch (kCycles) {
    case 2:
...
void Twofish::fill_keyed_sboxes(byte S[], int kCycles, TwofishKey* xkey) {
  int i;
...
void Twofish::InitKey(const byte key[], int key_len, TwofishKey* xkey) {
  byte K[32 + 32 + 4];
  int kCycles;
  int i;
  uint32_t A, B;
...
bool Twofish::Init(int key_bit_size, byte* key, int direction) {
  if (key_bit_size == 128) {
    ...
    LOG(ERROR) << "Twofish::Init bad key size\n";
    return false;
  }
  ...
  key_ = new byte[key_bit_size / num_bits_in_byte];
  if (key_ == nullptr) {
    LOG(ERROR) << "Twofish::Init key is nullptr\n";
    return false;
  }
  memcpy(key_, key, key_bit_size / num_bits_in_byte);
  initialise_q_boxes();
  initialise_mds_tables();
  InitKey(key, key_bit_size / num_bits_in_byte, &round_data);
  return true;
}
...
void Twofish::EncryptBlock(byte* p, byte* c) {
  volatile uint32_t A, B, C, D, T0, T1;
  const TwofishKey* xkey = &round_data;
...
void Twofish::DecryptBlock(byte* c, byte* p) {
  volatile uint32_t A, B, C, D, T0, T1;
  const TwofishKey* xkey = &round_data;
...
void Twofish::Encrypt(int size, byte* p, byte* c) {
  volatile uint32_t A, B, C, D, T0, T1;
  const TwofishKey* xkey = &round_data;
...
void Twofish::Decrypt(int size, byte* c, byte* p) {
  volatile uint32_t A, B, C, D, T0, T1;
  const TwofishKey* xkey = &round_data;
