...
string cryptoschemes[] = {"undefined",
...
string paddingalgs[] = {"undefined",
                        "none-symmetric-pad"
                        "standard-symmetric-pad"};

const int num_modes = 3;
string modes[] = {"undefined", "ecb", "cbc"};

const int num_hashalgs = 5;
string hashalgs[] = {
    "undefined", "sha1", "sha2-256", "sha2-512", "sha3-1024",
};
...
bool CryptoKey::SerializeKeyToMessage(crypto_key_message& message) {
  ...
  } else if (strcmp(key_type_->c_str(), "rsa") == 0 ||
...
bool CryptoKey::DeserializeKeyFromMessage(crypto_key_message& message) {
  ...
  if (message.has_not_before()) {
    ...
    if (!DecodeTime(message.not_before(), not_before_)) {
      LOG(ERROR) << "CryptoKey::DeserializeKeyFromMessage: cant decode "
                    "DecodeTime, not_before_\n";
      return false;
    }
  }
  if (message.has_not_before()) {
    ...
    if (!DecodeTime(message.not_after(), not_after_)) {
      LOG(ERROR) << "CryptoKey::DeserializeKeyFromMessage: cant decode "
                    "DecodeTime, not_after_\n";
      return false;
    }
  }
  if (message.has_symkey()) {
    crypto_symmetric_key_message* sc =
        (crypto_symmetric_key_message*)&message.symkey();
...
bool SymmetricKey::GenerateRc4Key(const char* name, const char* usage,
  ...
  if (num_bits != 128) {
    LOG(ERROR) << "SymmetricKey::GenerateRc4Key: unsupported key size "
               << num_bits << "\n";
    return false;
  }
  if (!GetCryptoRand(num_bits, my_key)) {
...
bool SymmetricKey::GenerateSimonKey(const char* name, const char* usage,
  ...
  if (num_bits != 128 && num_bits != 256) {
    LOG(ERROR) << "SymmetricKey::GenerateSimonKey: unsupported key size "
               << num_bits << "\n";
    return false;
  }
  if (!GetCryptoRand(num_bits, my_key)) {
...
bool SymmetricKey::SerializeKeyToMessage(
  ...
  string* s = ByteToBase64LeftToRight(symmetric_key_bit_size_ / NBITSINBYTE,
...
bool SymmetricKey::DeserializeKeyFromMessage(
    crypto_symmetric_key_message& message) {
  symmetric_algorithm_type_ = new string(message.key_type().c_str());
...
bool KeyStore::DeleteKey(const char* keyname) { return true; }

int KeyStore::NumKeys() { return num_keys_; }
...
bool Signature::Serialize(crypto_signature& sig) {
  ...
  if (size_serialized_ > 0 && serialized_statement_ != nullptr) {
    string* s =
        ByteToBase64LeftToRight(size_serialized_, serialized_statement_);
    if (s == nullptr)
      return false;
    sig.set_serialized_statement(s->c_str());
  }
  if (size_signature_ > 0 && signature_ != nullptr) {
    string* s = ByteToBase64LeftToRight(size_signature_, signature_);
    if (s == nullptr)
      return false;
...
bool Signature::Deserialize(crypto_signature& sig) {
  ...
  n = Base64ToByteLeftToRight((char*)sig.serialized_statement().c_str(), 512,
                              serialized_statement_);
  size_serialized_ = n;

  signature_ = new byte[512];
  n = Base64ToByteLeftToRight((char*)sig.signature().c_str(), 512, signature_);
  size_signature_ = n;
...
void Signature::PrintSignature() {
  ...
  if (size_serialized_ > 0 && serialized_statement_ != nullptr) {
    ...
    PrintBytes(size_serialized_, serialized_statement_);
    printf("\n");
  } else {
  ...
  if (size_signature_ > 0 && signature_ != nullptr) {
    printf("Signature: ");
    PrintBytes(size_signature_, signature_);
    printf("\n");
  } else {
    printf("No signature\n");
  }
