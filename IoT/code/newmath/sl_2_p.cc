  ...
  for (i = 0; i < n; i++) {
    ...
    for (int j = (i + 1); j < n; j++) {
      if (x[j].order_ != 8)
        continue;
      int* b_mat = x[j].mat_;
      if (!mult_mat(p, a_inv, b_mat, t1))
        return;
      if (!mult_mat(p, t1, a_mat, t2))
        return;
      if (is_equal(b_mat, t2)) {
        continue;
      }

      if (is_power(p, b_mat, t2)) {
        printf("---------\n");
...
void mult_order(int p, T* x, int n) {
  int r[4];
  for (int i = 0; i < n; i++) {
    if (x[i].order_ != 8)
      continue;
    for (int j = i+1; j < n; j++) {
      if (x[j].order_ != 8)
        continue;
      if (!mult_mat(p, x[i].mat_, x[j].mat_, r))
        return;
      int k = order(p, r);
      printf("(%d, %d): %d\n", i, j, k);
    }
...
int main(int an, char** av) {
  ...
  for (int i = 0; i < an; i++) {
    ...
    if (strncmp(av[i], "--mult_order", n) == 0) {
      mult_orders = true;
    }
  }
