...
{
	unsigned j,k,x,basesize,first,nfixpts,*perm;
		...
		for (j = 0; j < orbsize; ++j)   {
			...
			for (gd = top[i]; gd->pgen != NULL; gd = gd->next)   {
				...
				if (gd->ck <= j) {
					...
					++(gd->ck);
					if (pg !=  NULL) {
					/* a new generator has been found, so */
						first = i+1;
...
new:    
	if (pg != NULL) {
		...
		if (nfixpts >= basesize) {
			++basesize;
			perm = pg->g;
			for ( j=0; perm[j] == bigbase[j]; ++j );
			base[basesize-1] = bigbase[j];
