...
Let $n= {p_1}^{e_1} {p_2}^{e_2} \ldots {p_k}^{e_k}$. $f$ is \emph{multiplicative}
if $(m,n)=1$ $\rightarrow$ $f(mn)=f(m)f(n)$.
$\mu(1)= 1$, if $n=1$; $\mu(n)=1$.  If 
$n= {p_1}^{e_1} {p_2}^{e_2} \ldots {p_k}^{e_k}$.
$e_i > 1$ for any $i$, $\mu(n)=0$; otherwise, $\mu(n)= (-1)^k$.
\\
\\
{\bf Theorem:} 
...
If ${\frac {1} {p}} + {\frac {1} {q}} = 1$ then (a)
${\frac {a^p} {p}} + {\frac {b^q} {q}} \geq ab$ and
(b) $(\sum_{i} {a_i}^p )^{\frac 1 p} \cdot (\sum_{i} {b_i}^q )^{\frac 1 q}  \geq
\sum_{i} a_i b_i $.
\begin{quote}
\emph{Proof:} If ${\frac {1} {p}} + {\frac {1} {q}} = 1$ then $p + q =pq$ and $q=p(q-1)$
so $q-1= {\frac q p}$.  Similarly, $p-1= {\frac p q}$.  Thus $p-1 = {\frac 1 {q-1}}$.
Let $f(x)= x^{p-1}$.  $f^{-1}(y) = y^{\frac 1 {p-1}}=y^{q-1}$.  The rectangle with diagonal
points $(0,0$ and $(a,b)$.  The area of the rectangle is $\int_0^a f(x) dx + \int_0^b f^{-1}(y) dy$.
$[{\frac {x^p} p}]_0^a + [{\frac {y^q} q}]_0^b \geq ab$.
If $f$ is
monotonically increasing, $f(0)= 0$, then $\int_0^a f + \int_0^b f^{-1} \geq
ab$.
\\
\emph{Another proof:}  You can prove first part using Arithmetic-Geometric inequality, as follows.
\\
$p+q=pq$.  By AM, ${\frac 1 {pq}} (qa^p + p b^q)={\frac 1 {p+q}} (qa^p + p b^q) \geq (a^{pq}b^{pq})^{\frac 1 {pq}}$.
So, ${\frac {a^p} {p}} + {\frac {b^q} {q}} \geq ab$.
\\
\\
For (b), apply this inequality repeatedly with
...
\begin{quote}
\emph{Proof:}  ${\frac 1 p} + {\frac {p-1} p} =1$, so we can apply Holder
with $q= {\frac p {p-1}}$.  Write
$$
(x_1+x_2)^p +(y_1+y_2)^p
= [(x_1+x_2)^{p-1}x_1 +(y_1+y_2)^{p-1} y_1]
+ [(x_1+x_2)^{p-1}x_2 +(y_1+y_2)^{p-1} y_2]$$  Apply Holder with
$a_1=x_1, a_2=y_1$ and $b_1=(x_1 + x_2)^{p-1}, b_2= (y_1 + y_2)^{p-1}$ to get
$$
x_1 (x_1 + x_2)^{p-1} + y_1 (y_1 + y_2)^{p-1} \leq ({x_1}^p + {y_1}^p)^{\frac 1 p} ((x_1 + x_2)^{q(p-1)} + (y_1+y_2)^{q(p-1)})^{\frac 1 q}
$$
and then Holder with $a_1=x_2, a_2=y_2$ and $b_1=(x_1 + x_2)^{p-1}, b_2= (y_1 + y_2)^{p-1}$ to get
$$
x_2 (y_1 + y_2)^{p-1} + y_2 (x_1 + x_2)^{p-1} \leq ({x_2}^p + {y_2}^p)^{\frac 1 p} ((y_1 + y_2)^{q(p-1)} + (x_1+x_2)^{q(p-1)})^{\frac 1 q}
$$
Add these two to get
$$
(x_1 + x_2)^p + (y_1 + y_2)^p  \leq ((x_1 + x_2)^{q(p-1)} + (y_1+y_2)^{q(p-1)})^{\frac 1 q} ({x_1}^p + {y_1}^p)^{\frac 1 p} + ({x_2}^p + {y_2}^p)^{\frac 1 p}
$$
Since $q(p-1)=p$, dividing by $((y_1 + y_2)^{q(p-1)} + (x_1+x_2)^{q(p-1)})^{\frac 1 q}$, we get
$$
{\frac
{ (x_1 + x_2)^p + (y_1 + y_2)^p)}
{((x_1 + x_2)^p + (y_1 + y_2)^p)^{\frac 1 q}}
}=
((x_1 + x_2)^p + (y_1 + y_2)^p)^{1-{\frac 1 q}} = ((x_1 + x_2)^p + (y_1 + y_2)^p)^{\frac 1 p}  \leq ({x_1}^p + {y_1}^p)^{\frac 1 p} +({x_2}^p + {y_2}^p)^{\frac 1 p}
$$
\end{quote}
...
\\
\\
{\bf Symmetry group of Rubik's cube:} $|G_R|= 2^{27}3^{14}5^3 7^2 11 = {\frac
{8! \cdot 3^8 \cdot 12! \cdot 2^{12}} {2 \cdot 2 \cdot 3}}$.
Cycle corners: $fUBuFUbu$.
Rotate cornet cubie: $LdlfdFUfDFLDlu$.
Flip adjacent edge cubies: $FRBLUlUbrfluLu$.
Rotate corner cubies: $FDfdFDf$.
Cycle edge cubies: $BULulb$.
\\
\\
{\bf Some Relations:}
...
\\
Number of ways to distribute $r$ distinct objects into $n$ distinct cells (more than one element in cell allowed): 
${\frac {(n+r-1)!} {(n-1)!}}$.
\\
\\
{\bf Least squares line fitting:} Given points 
$(x_1, y_i), i = 1, 2, \ldots, n$, the least squares line $y=mx+b$ to fit the points is:
$b = {\frac {\sum_{i=1}^n y_i - m \sum_{i=1}^n x_i} {n}}$ ,
$m = {\frac {n \sum_{i=1}^n x_i y_i -  (\sum_{i=1}^n x_i) (\sum_{i=1}^n y_i)}
{n \sum_{i=1}^n {y_i}^2 -  (\sum_{i=1}^n {x_i}^2)}}$ 
